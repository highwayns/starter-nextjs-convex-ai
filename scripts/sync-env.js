 ...
 *
 * Features:
 * - Parses human-readable environment file from ~/.env-configs/
...
const WORKER_ENV_FILE = path.join(
  ROOT_DIR,
...
if (!deploymentArg) {
  ...
  console.error('Usage:');
  console.error(
    '  bun run sync-env --deployment=dev      # For development environment'
  );
  console.error(
    '  bun run sync-env --deployment=production # For production environment (blocked by safety)'
  );
  console.error('');
...
function parseSourceFile() {
  ...
  if (!fs.existsSync(SOURCE_FILE)) {
    ...
    console.log(
      `💡 Please create ${SOURCE_FILE} with your environment configuration`
    );
    console.log(
      `💡 Or run: mkdir -p ${ENV_CONFIG_DIR} && cp .env.source-of-truth.local ${SOURCE_FILE}`
    );
    process.exit(1);
  }
...
function generateNextjsEnv(envVars) {
  ...
  content +=
    '# =============================================================================\n';
  content += `# Auto-generated from ${SOURCE_FILE} - DO NOT EDIT MANUALLY\n`;
  content +=
    "# Run 'npm run sync-env' or 'bun run sync-env' to regenerate this file\n";
  content += '#\n';
  content +=
    '# This file contains environment variables specific to the Next.js application.\n';
  content +=
...
function generateConvexEnv(envVars) {
  ...
  if (convexVars.length === 0) {
    console.log(
      'ℹ️  No Convex-specific variables found, skipping Convex env generation'
    );
    return 0;
  }
  ...
  content += '#\n';
  content +=
    '# This file contains environment variables specific to the Convex backend.\n';
  content +=
    '# These variables are used for server-side operations and API integrations.\n';
  content +=
    '# =============================================================================\n\n';
...
function generateWorkerEnv(envVars) {
  ...
  if (workerVars.length === 0) {
    console.log(
      'ℹ️  No Worker-specific variables found, skipping Worker env generation'
    );
    return 0;
  }
  ...
  content += '#\n';
  content +=
    '# This file contains environment variables for local Worker development.\n';
  content +=
    '# For production deployment, use: wrangler secret put VARIABLE_NAME\n';
  content +=
    '# =============================================================================\n\n';
  ...
  envVars.forEach(envVar => {
    ...
    if (envVar.key.startsWith('NEXT_PUBLIC_')) {
      ...
      if (
        ...
        errors.push(
          `🚨 SECURITY: ${envVar.key} appears to contain sensitive data but is marked as public`
        );
      }
    }
    ...
    if (
      envVar.key === 'CONVEX_DEPLOYMENT' &&
      (!envVar.convex || !envVar.nextjs)
    ) {
      warnings.push(
        `CONVEX_DEPLOYMENT should typically be available to both Next.js and Convex`
      );
    }
  });
...
function validateDeployment(deployment) {
  ...
  if (deployment.includes('prod') || deployment.includes('production')) {
    console.error(
      '🚨 SECURITY: Production deployments must be managed manually for security'
    );
    console.error(
      '   Use manual commands: bunx convex env set --prod VARIABLE value'
    );
    process.exit(1);
  }
  ...
  if (deployment.includes('preview')) {
    console.warn(
      '⚠️  Targeting preview deployment - ensure this is intentional'
    );
  }
...
function backupConvexEnvironment(deployment) {
  ...
  try {
    ...
    const timestampedBackupFile = path.join(
      BACKUP_DIR,
      `.env.backup.${timestamp}.local`
    );
    ...
    // Also save to current backup file for easy access
    const currentBackupFile = path.join(
      BACKUP_DIR,
      '.env.backup.current.local'
    );
    fs.writeFileSync(currentBackupFile, backupContent);

    console.log(`✅ Environment backup saved to: ${timestampedBackupFile}`);
    console.log(`   (Also available as: ${currentBackupFile})`);
...
function showExistingBackups() {
  try {
    ...
    if (files.length > 1) {
      // More than just the current one
...
function displayEnvironmentDiff(diff) {
  ...
  if (diff.toUpdate.length > 0) {
    ...
    diff.toUpdate.forEach(envVar => {
      const oldPreview =
        envVar.oldValue.length > 30
          ? envVar.oldValue.substring(0, 27) + '...'
          : envVar.oldValue;
      const newPreview =
        envVar.newValue.length > 30
...
function applyConvexEnvironmentChanges(diff, deployment) {
  ...
  diff.toAdd.forEach(envVar => {
    try {
      ...
      execConvexCommand(`bunx convex env set ${envVar.key} "${envVar.value}"`, {
  ...
  diff.toUpdate.forEach(envVar => {
    try {
      ...
      execConvexCommand(
        `bunx convex env set ${envVar.key} "${envVar.newValue}"`,
        { silent: !options.verbose }
      );
      successCount++;
    } catch (error) {
  ...
  diff.toRemove.forEach(envVar => {
    try {
      ...
      execConvexCommand(`bunx convex env remove ${envVar.key}`, {
        silent: !options.verbose,
      });
      successCount++;
    } catch (error) {
...
function syncConvexDeploymentEnv(envVars, deployment) {
  ...
  if (results.failureCount === 0) {
    ...
    const finalChanges =
      finalDiff.toAdd.length +
      finalDiff.toUpdate.length +
      finalDiff.toRemove.length;

    if (finalChanges > 0) {
      console.warn(
        '⚠️  Warning: Some changes may not have been applied correctly'
      );
      console.warn(
        '   Run the sync command again or check manually with: bunx convex env list'
      );
    }
  } else {
...
function getCurrentWorkerSecrets() {
  try {
    const secretList = execWranglerCommand('wrangler secret list', {
      silent: true,
      allowFailure: true,
    });
...
function syncWorkerSecrets(envVars, deployment) {
  ...
  console.log(
    `ℹ️  Current Worker has ${currentSecrets.length} secrets configured`
  );
  ...
  workerVars.forEach(envVar => {
    try {
      ...
      execWranglerCommand(
        `echo "${envVar.value}" | wrangler secret put ${envVar.key}`,
        { silent: !options.verbose }
      );
      successCount++;
    } catch (error) {
      console.error(
        `❌ Failed to set Worker secret ${envVar.key}:`,
        error.message
      );
      failureCount++;
    }
...
function main() {
  ...
  try {
    ...
    console.log('\n' + '='.repeat(60));
    const deploymentResult = syncConvexDeploymentEnv(
      envVars,
      options.deployment
    );
    ...
    console.log(
      `   • Convex deployment sync: ${deploymentResult === 0 ? 'Success' : 'Failed'}`
    );
    console.log(
      `   • Worker secrets sync: ${workerResult === 0 ? 'Success' : 'Failed'}`
    );
    console.log('');
    ...
    console.log(`   • ${BACKUP_DIR}/.env.backup.current.local (backup)`);
    console.log('');
    ...
    console.log(`   • Keep ${SOURCE_FILE} secure and update as needed`);
